...
const InquiryPage = ({ params }: { params: { id: string } }) => {
    ...
    useEffect(() => {
        if (params.id) {
            const fetchInquiry = async () => {
                try {
                    const response = await apiService.getWithToken(`/api/inquiries/get/${params.id}`);
                    setInquiry(response);
                } catch (error) {
    ...
    const handleSave = async () => {
        ...
        setIsLoading(true);
        try {
            await apiService.postWithoutImages(`/api/inquiries/add-message/${inquiry?.id}/`, { message: response, sender: 'customer_service' });
            setResponse('');
            setInquiry((prev) => prev && { ...prev, messages: [...prev.messages, { sender: 'customer_service', message: response, timestamp: new Date().toISOString() }] });
        } catch (error) {
    ...
    const toggleStatusDetails = () => setShowStatusDetails((prev) => !prev);
